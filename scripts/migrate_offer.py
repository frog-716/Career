#!/usr/bin/env python3
"""Explicit isolated v5→v6 Offer runtime fence; production cutover is unsupported."""
import argparse
import json
import os
import sqlite3
...
from workbench.offer_migration import apply_migration, dry_run, verify_migration
    ...
    try:
        ...
        with os.fdopen(os.open(output, os.O_WRONLY | os.O_CREAT | os.O_EXCL, 0o600), "w") as stream:
            try:
                ...
                json.dump({"status": "failed", "error": str(error)}, stream, ensure_ascii=False)
                raise
        print(json.dumps({"report": str(output.resolve()), "production_cutover": False}))
        return 0 if result.get("verified", True) else 2
    except (Invalid, Conflict, ValueError, OSError, sqlite3.Error) as error:
