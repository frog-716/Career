...
def test_typed_activity_keeps_raw_note_and_is_restartable(tmp_path):
    ...
    job = post(c, "/jobs", {"company": "虚构公司", "title": "岗位", "jd": "JD"}).json()
    second = post(c, "/jobs", {"company": "另一公司", "title": "岗位", "jd": "JD2"}).json()
    ...
    assert len(activity["communications"]) == len(activity["interviews"]) == len(activity["offers"]) == 1
    ...
    assert {raw["content"] for raw in raws} >= {"研究原文", "沟通原文", "面试原文", "Offer 原文"}
