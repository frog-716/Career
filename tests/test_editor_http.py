...
def test_editor_route_guard_and_conflict_preserve_current(tmp_path):
    ...
    assert client.put('/api/editor', json=body).status_code == 403
    headers = {'X-Career-Request': '1'}
    assert client.put('/api/editor', json=body, headers={
        **headers, 'Origin': 'https://untrusted.example'
    }).status_code == 403
    saved = client.put('/api/editor', json=body, headers=headers)
    assert saved.status_code == 200
    body['document']['profile']['name'] = '过期窗口候选人乙'
    assert client.put('/api/editor', json=body, headers=headers).status_code == 409
    assert client.get('/api/editor').json()['document']['profile']['name'] == '虚构候选人甲'
