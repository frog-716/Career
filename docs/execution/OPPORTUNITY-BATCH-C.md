...
状态：**Plan only / 待确认实施**。日期：2026-09-17。按 `$workbench-plan` 编制；本轮未调用 `$workbench-implement`，未修改业务代码、执行测试、创建备份、迁移或切换服务。下文“应当/必须/验收”均为预先固定的实施条件，不是实现声明。
...
[Batch B §11](OPPORTUNITY-BATCH-B.md)是历史验收证据；本轮直接核对了其源码、manifest、inventory 和最终 migration verify，未重新查询生产进程。B 当时生产为 `/Users/frog/Projects/Career` 的 0.2.0 / schema v1，数据为 `/Users/frog/Library/Application Support/Career Data`；**不是本轮重新测得的在线状态**。C 实施前重新确认服务、源码、数据、附件和端口。禁止 Production Cutover，禁止重启/替换生产，禁止改变生产正式写路径。
...
2. 点“制作简历”，明确选择已有结构化版本、legacy 工作稿、受支持 JSON 或空白；创建本机会唯一工作稿。另一机会重复操作得到另一份独立稿。
...
| **Verified by code**：附件原子写在版本 DB 事务内，异常尝试 unlink；删除只检查 applications.version_id / ResumeUse | 增加本批文件操作恢复日志、引用全集保护和 crash 验证；不扩展通用文件平台 |
...
- `structured_json`：仅支持本项目导出的 schemaVersion=1 结构，遵守现有大小/类型/ID/链接校验；不支持 PDF/Word/OCR/任意文本还原。来源元数据不能伪造已确认 Wiki 引用或其他 owner，合法结构复制后独立。这是需要新增的窄导入路径，当前前端只有导出、没有导入。

复制保留结构/排版及内容内部 ID（以 document 为命名空间），源链接另存不可变 provenance。来源的个人引用可保留来源痕迹；来自其它机会的 scope 引用只能标为 `copied_source` 的历史来源，不能作为目标机会已确认事实、有效选材授权或自动 Context。外部 JSON 的 source_refs 不作为服务器认证来源。内容是用户明确复制的表达，不因此写回 Wiki。
...
C 只提供投递前当前 Greeting 编辑。投递后材料面板只读 snapshot，不暴露修改历史入口；以后若允许改当前 Greeting，也不得改变 snapshot，本批不加该入口。新空机会缺字段等价“尚未准备”，不必初始化写入。
...
4. 按 §8 准备可恢复的最终 PDF；仅 version 首次复制分支可按显式来源创建文档。生成新的 submission_id、特殊 vid、aid；模式 none 完全跳过它们。
...
6. COMMIT 后返回 `{submission, opportunity, submission_version?, resume_document?}`；完成文件日志收尾。任何成功响应只依据已提交记录。提交成功但响应丢失，前端保留同键重试；不重新渲染生成另一个 PDF 请求。
...
现有 `try/unlink` 不能覆盖进程崩溃，C 增加仅用于本批版本/PDF的持久文件操作日志（如数据目录内 `artifact-operations/`），不创建第二套 Submission 仓库：

- 日志包含 operation_id、action fingerprint、目标 aid/vid、相对路径、sha256、操作类型；不复制个人正文。stage PDF 与日志先 fsync，最终 rename 及目录 fsync 在持有 DB 写锁的窗口完成，然后提交业务事务。
- 一般异常回滚后核实 DB 引用再处理本操作文件，不能在“COMMIT已成功、返回失败”时 unlink 成功材料。文件系统与SQLite不是同一原子存储，日志负责补齐崩溃边界。
- 重启在接收写入前，以同一DB写锁恢复本日志：已提交且hash正确则保留文件/补收尾；未提交且只有本operation拥有则退回隔离暂存区；hash不符/引用不明则停止相关写入并报告，不删文件、不猜补 Submission。
- 正常受控失败后正式 `artifacts/` 不留无引用文件；强制崩溃后首次恢复前允许日志标识的 pending 文件，恢复后正式引用集合一致。保留的失败暂存/隔离文件是明确未提交操作材料，不显示成业务版本；不自动扫描删除旧文件。
- 备份须与该写锁/恢复协议一致：有未结算日志时明确拒绝并要求先恢复，不带着未知半状态报告成功；已结算隔离暂存不当作已提交PDF，数量单独报告，原文件保留。备份验证仍要求所有已提交 Artifact 文件存在且hash一致。
...
未被引用的普通版删除也使用上述操作日志：DB删除提交前不得不可恢复地删PDF；提交后可移入操作隔离区并完成收尾，崩溃可判定恢复。若发现同一路径/aid被其它记录引用，绝不能 unlink。特殊版即使异常缺少反向 application，也按 version_kind 永久拒绝删除。C 不提供清空工作稿所有权、清理legacy或垃圾回收 UI。
...
| C3 文档owner和文件边界 | `editor.py`；可新增窄职责 `resume_documents.py`、`artifact_operations.py`；`artifacts.py` 的复用边界；`backup.py` | scoped draft/版本/选材/恢复，受控PDF登记/删除恢复；不改截图、旧文本renderer的产品行为 |
...
**仅后续实施时执行以下动作；本轮不执行。** schema3不等于批准生产升级。CLI沿用B的系统临时目录allowlist，解析realpath并拒绝生产目录、现有目标、代码目录、DB符号链接/硬链接、被链接的附件目录、报告覆盖。迁移不得直接作用于唯一备份。
...
9. v3已经写入后：暂停该隔离副本写入并备份当前v3，restore到又一个新目录，用匹配C或修正后的v3程序forward recovery；新普通版/特殊版、无简历Submission、Greeting、阶段/CAS/幂等记录全部保全，再验证同键回放与文件日志恢复。不得恢复pre-C快照丢弃这些新增事实。不承诺无损逆迁移，C不实现生产部署动作。
...
| 编号 | 固定测试/断言 | 对应要求 |
| --- | --- | --- |
| T01 owner隔离 | 两机会懒创建各0..1；所有4种显式source；打开不创建；第二创建并发/同键回放；跨did/伪造owner拒绝；复制源owner/ResumeUse不变 | O04 |
...
| T09不变性/引用 | 提交后改稿/Profile/Wiki/JD/恢复，旧snapshot/version/PDF恒等；特殊版UPDATE/DELETE拒绝；所有artifact读取以引用为准；人为共享aid/path与历史用途fixture证明删除保护全集 | O07 |
...
| 编号 | 实际操作与应观察结果 |
| --- | --- |
| U01 | 新建A/B两机会不建稿；从首次来源选择分别建稿，空白/结构JSON/legacy/已有版4种来源均实际走通；无效JSON清楚报错且不留半个稿 |
...
| U08 | 投递后改当前稿/Profile/JD再回材料面板，原PDF/Greeting/正文不变；版本列表只在Resume Workspace，不能删除特殊版；旧投递缺Greeting显示“历史未记录” |
...
普通工程选择不设用户Gate：current JSON复用、schema3编号、文件副本与日志、Greeting字段、scoped API、来源导入校验。实现如发现真正与目标冲突，先给出具体条款和复现，再调整计划，不能以额外测试单独定义成功。

尚未验证：C所有代码/测试/浏览器/迁移/recovery都未执行；生产当前进程/数据是否较B变化未重测；完整客户端残余引用扫描需实施后复查；新文件日志在真实子进程中断后的表现待测；最新备份是否增加多投递待新inventory。真实AI、外部投递送达证明、任意PDF导入、ATS文字解析、全D–G、异机灾备和无损v3→v2逆迁移不在本批验收。
...
| 半个Submission/版本/孤儿PDF | 单SQLite业务事务+可恢复文件日志；COMMIT后失联不得删PDF；T10的真实crash与恢复，不只mock异常 |
...
再核对：没有为命名新建Submission表，没有把ResumeUse变owner，没有两个可写Opportunity正本；创建稿与普通保存不改phase；legacy application.status、plan.closed、job.deleted/excluded与已有Interview/Offer都不能生成result或猜历史phase。文件日志仅记录操作恢复，不构成事件总线或新业务事实库。

## 16. 明确停止点

**本轮到正式计划完成即停止。** 仅新增本文件、更新STATUS当前规划入口；由于B实现与生产源码分居两个工作树，将这两个规划文件同步到B工作树以便后续实施读取。它们是同一计划的工作树副本，不是新的审计正本；不修改A/B验收记录、审计或target。未启动workbench-implement，未运行C自动测试/浏览器/备份/迁移。

未来若明确授权实施：按C1–C7完成后用workbench-review核对本计划的需求和工程约束；所有预定测试、真实浏览器、最新生产备份隔离迁移、旧程序拒绝、rollback/forward恢复均有实际证据才能报告“Batch C隔离验收通过”。如果关键项失败，明确未完成，不用已有B成功替代。通过后仍停止，不进入D/E/F/G、不执行Production Cutover、不改生产v1/正式写路径，不接真实AI。

Production Cutover是后续单独授权的部署动作。C完成也不能让生产失去尚未由后续批次接管的沟通、面试、Offer能力。本计划不授权该部署。

本轮文档核验：以开工文件hash清单比较，原工作树211个、B工作树218个已纳入Git清单的文件中，仅本计划（新增）和STATUS发生变化；两处计划副本一致，本地链接、代码围栏、空白检查通过。此核验只证明文档改动范围，不证明C业务能力通过。
