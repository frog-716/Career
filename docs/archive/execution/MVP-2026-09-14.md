...
> 2026-09-14 纠偏：本文以下记录已有技术实现。简历纯文本 Interface 不再作为目标编辑器契约；应依照 docs/01-product.md 的既有工作台基线及 docs/03-architecture.md 的结构化接入约束修订后续批次。当前状态以 STATUS.md 为准。
...
Python 3.9 + FastAPI + SQLite 单库（逻辑隔离正本/分析），原子附件；TypeScript + Vite vanilla UI，无云部署。生产默认 ~/Library/Application Support/Career Data，CAREER_DATA_DIR 必须在代码目录外。测试使用临时目录。服务仅 127.0.0.1:8765。
