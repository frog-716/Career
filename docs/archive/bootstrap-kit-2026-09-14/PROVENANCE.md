...
启动包曾包含 `reference_code/sqlite_archive.py`：它根据旧项目 `career-hub/career_hub/database.py` 中backup/restore方法重写成独立函数，去掉旧schema、类依赖和路径绑定；补充源文件只读、目标排他创建、完整性检查。它只处理一个SQLite文件，后来已由当前备份模块取代，归档副本已删除。

参考测试在临时目录使用虚构内容；当时运行结果记录在PACKAGING-REPORT。它不属于当前系统已实现功能，代码与测试副本已经删除。
